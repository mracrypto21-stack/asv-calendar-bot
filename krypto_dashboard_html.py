...
Katrai no 8 kartītēm ir īsts SVG grafiks ar datiem (sparkline, donut,
spidometrs, joslas, progresa joslas). 1600x900, 4x2 režģis, bez avotu teksta.
...
def sparkline(series, w, h, color, fill=True, highlow=False):
    """7 dienu sparkline ar gradienta aizpildījumu un mirdzošu beigu punktu."""
    ...
    area = f"{px(0):.1f},{h} " + pts + f" {px(len(vals)-1):.1f},{h}"
    ...
    hi_x = px(hi_i)
    lo_x = px(lo_i)
    last_x = px(len(vals) - 1)
    last_y = py(vals[-1])
    hl = ""
    if highlow:
        hl = (f'<line x1="{hi_x}" y1="{hi_y}" x2="{hi_x}" y2="{h}" stroke="{color}" stroke-opacity="0.25" stroke-width="1" stroke-dasharray="3,3"/>'
              f'<line x1="{lo_x}" y1="{lo_y}" x2="{lo_x}" y2="{h}" stroke="{color}" stroke-opacity="0.25" stroke-width="1" stroke-dasharray="3,3"/>'
              f'<circle cx="{hi_x}" cy="{hi_y}" r="3" fill="{color}"/>'
              f'<circle cx="{lo_x}" cy="{lo_y}" r="3" fill="{color}"/>')
    return f"""
    <svg width="{w}" height="{h}" viewBox="0 0 {w} {h}" style="display:block;">
      ...
      {hl}
      <circle cx="{last_x}" cy="{last_y}" r="5" fill="{color}"
        style="filter:drop-shadow(0 0 8px {color});"/>
    </svg>"""


def donut(btc, eth, w, h, center_text):
    """Donut diagramma BTC / ETH / Others ar % centrā."""
    ...
    labels = ""
    lx, ly = cx, cy - rr - 14
    labels += f'<text x="{cx}" y="{ly}" text-anchor="middle" font-size="13" font-weight="700" fill="#ffb300">BTC {btc:.1f}%</text>'
    labels += f'<text x="{cx}" y="{ly+18}" text-anchor="middle" font-size="12" font-weight="600" fill="#b44dff">ETH {eth:.1f}%</text>'
    return f"""
    <svg width="{w}" height="{h}" viewBox="0 0 {w} {h}" style="display:block;">
      {''.join(arcs)}
      <text x="{cx}" y="{cy+6}" text-anchor="middle" font-size="26" font-weight="900" fill="#ffffff"
        style="filter:drop-shadow(0 0 8px #00e5a0);">{center_text}</text>
      {labels}
    </svg>"""


def gauge(fng, fng_prev, w, h):
    """Pusloka spidometrs ar krāsu skalu un adatu + iepr. nedēļas marķieri."""
    cx, cy = w / 2, h - 8
    r = min(w, h * 2) / 2 - 6
    # krāsu skala 0..100 pa pusloku (180°)
    stops = [
        (0.0, "#ff3b5c"),   # Extreme Fear
        (0.25, "#ff8a3d"),  # Fear
        (0.5, "#ffd700"),   # Neutral
        (0.75, "#00e5a0"),  # Greed
        (1.0, "#00b36b"),   # Extreme Greed
    ]
    def angle(v):
        # 0 → 180°, 100 → 0° (pusloks no kreisās uz labo)
        return 180 - (v / 100.0) * 180
    def pt(v, rad):
        a = angle(v) * 3.14159265 / 180
        return (cx + rad * __import__("math").cos(a), cy - rad * __import__("math").sin(a))
    # skala: 5 segmenti
    segs = ""
    for i in range(5):
        v0, v1 = i * 20, (i + 1) * 20
        col = stops[i][1]
        a0 = angle(v0) * 3.14159265 / 180
        a1 = angle(v1) * 3.14159265 / 180
        large = 1 if (v1 - v0) > 180 else 0
        ...
        segs += f'<path d="M {x0:.1f} {y0:.1f} A {r:.1f} {r:.1f} 0 {large} 1 {x1:.1f} {y1:.1f}" fill="none" stroke="{col}" stroke-width="14" stroke-linecap="butt" style="filter:drop-shadow(0 0 5px {col}66);"/>'
    # adata uz šodienas vērtību
    ax, ay = pt(fng, r - 16)
    ...
    # iepriekšējās nedēļas marķieris
...
def bars(prev, this, daily, w, h, color):
    """Joslu diagramma: Prev week vs This week + 5 dienu joslas."""
    # galvenās 2 joslas
    ...
    def bar(x, val, col, label):
        bh = abs(val) / maxv * yh
        yy = y0 + yh - bh if val >= 0 else y0 + yh
        return (f'<rect x="{x}" y="{yy:.1f}" width="{bw}" height="{bh:.1f}" rx="4" fill="{col}" '
                f'style="filter:drop-shadow(0 0 6px {col}66);"/>'
                f'<text x="{x+bw/2}" y="{yy-6 if val>=0 else yy+bh+14}" text-anchor="middle" font-size="12" font-weight="700" fill="{col}">{label}</text>')
    pcol = "#3a4a6a"
    tcol = color
    main = ""
    main += bar(gap, prev, pcol, "Prev")
    main += bar(gap + bw + gap, this, tcol, "This")
    # 5 dienu joslas apakšā
    dy = y0 + yh + 26
    dh = h - dy - 4
    dmax = max([abs(x) for x in daily] + [1.0])
    dbar_w = (w - 2 * gap) / 5 - 4
    daily_svg = ""
    for i, v in enumerate(daily):
        ...
        bh = abs(v) / dmax * dh
        x = gap + i * (dbar_w + 4)
        yy = dy + dh - bh if v >= 0 else dy + dh
        daily_svg += f'<rect x="{x:.1f}" y="{yy:.1f}" width="{dbar_w:.1f}" height="{bh:.1f}" rx="3" fill="{col}" style="filter:drop-shadow(0 0 4px {col}66);"/>'
    return f"""
    <svg width="{w}" height="{h}" viewBox="0 0 {w} {h}" style="display:block;">
      {main}
      {daily_svg}
...
def progress_rows(rows, w, h):
    """Weekly Change Summary: horizontālas progresa joslas 5 rādītājiem."""
    items = []
    n = len(rows)
    row_h = (h - 20) / n
    for i, (label, val_str, val_num, up) in enumerate(rows):
        ...
        pct = min(100.0, abs(val_num) * 2.0)
        items.append(f"""
        <g>
          <text x="0" y="{y+14}" font-size="13" font-weight="700" fill="#cfe0ff">{label}</text>
          <rect x="0" y="{y+18}" width="{w}" height="10" rx="5" fill="rgba(255,255,255,0.10)"/>
          <rect x="0" y="{y+18}" width="{w*pct/100:.1f}" height="10" rx="5" fill="{col}" style="filter:drop-shadow(0 0 5px {col}88);"/>
          <text x="{w}" y="{y+14}" text-anchor="end" font-size="13" font-weight="800" fill="{col}">{val_str}</text>
        </g>""")
...
def build_html(d, bg_data_uri=None):
    ...
    # Grafiki
    chart_w, chart_h = 300, 120
    mc_chart = sparkline(d.get("total_cap_series", []), chart_w, chart_h, "#00d4ff", fill=True)
    btc_chart = sparkline(d.get("btc_series", []), chart_w, chart_h, "#ffb300", fill=True, highlow=True)
    eth_chart = sparkline(d.get("eth_series", []), chart_w, chart_h, "#b44dff", fill=True, highlow=True)
    dom_chart = donut(btc_dom, eth_dom, chart_w, chart_h, f"{btc_dom:.1f}%")
    fng_chart = gauge(fng, fng_prev, chart_w, chart_h)
    btc_etf_chart = bars(btc_etf_prev, btc_etf_this or 0, btc_etf_daily, chart_w, chart_h, "#ff8a3d")
    eth_etf_chart = bars(eth_etf_prev, eth_etf_this or 0, eth_etf_daily, chart_w, chart_h, "#7a5cff")
    ...
    def card(title, chart, value, sub, up, accent, glow, icon, value_red=False):
    ...
    def icon_svg(color, kind):
        if kind == "cap":
            return f'<svg width="20" height="20" viewBox="0 0 24 24" fill="none"><path d="M3 17l6-6 4 4 8-8" stroke="{color}" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"/><path d="M15 7h6v6" stroke="{color}" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"/></svg>'
        if kind == "btc":
        ...
        if kind == "dom":
            return f'<svg width="20" height="20" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="9" stroke="{color}" stroke-width="2.5"/><path d="M12 3v9l6 3" stroke="{color}" stroke-width="2.5" stroke-linecap="round"/></svg>'
        if kind == "fng":
            return f'<svg width="20" height="20" viewBox="0 0 24 24" fill="none"><path d="M12 3a9 9 0 100 18 9 9 0 000-18z" stroke="{color}" stroke-width="2.5"/><path d="M8 14l2-2 2 2 4-4" stroke="{color}" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"/></svg>'
        if kind == "etf":
            return f'<svg width="20" height="20" viewBox="0 0 24 24" fill="none"><rect x="3" y="4" width="18" height="16" rx="3" stroke="{color}" stroke-width="2.5"/><path d="M8 12h8M8 16h5" stroke="{color}" stroke-width="2.5" stroke-linecap="round"/></svg>'
        if kind == "sum":
            return f'<svg width="20" height="20" viewBox="0 0 24 24" fill="none"><path d="M4 20V10M10 20V4M16 20v-8M22 20H2" stroke="{color}" stroke-width="2.5" stroke-linecap="round"/></svg>'
        return ""
...
<style>
  * {{ margin:0; padding:0; box-sizing:border-box; }}
  body {{
    ...
    color:#f4f6ff; padding:34px 40px 30px;
  ...
  .card-title {{ font-size:15px; color:#dbe7ff; font-weight:700; line-height:1.2; }}
  .card-icon {{
    width:32px; height:32px; border-radius:9px; flex-shrink:0;
    display:flex; align-items:center; justify-content:center;
    box-shadow:0 0 14px var(--glow), inset 0 1px 0 rgba(255,255,255,0.4);
  }}
